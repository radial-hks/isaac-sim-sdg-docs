...
âââ config/extension.toml          â metadata, dependencies, test entries
âââ nodes/
â   âââ OgnExampleCpp.ogn          â rename/replace with your IPC node schema
â   âââ OgnExampleCpp.cpp          â rename/replace with your IPC node implementation
âââ plugins/<extension_name>/
â   âââ PluginInterface.cpp        â Carbonite plugin + OGN registration (keep as-is)
âââ bindings/<extension_name>/
â   âââ Bindings.cpp               â pybind11 bindings; acquires the Carbonite interface (keep as-is)
âââ include/<extension_name>/
â   âââ IExampleNodes.h            â Carbonite interface (keep as-is)
âââ python/nodes/
â   âââ OgnExamplePython.ogn       â rename/replace with your Python node schema
â   âââ OgnExamplePython.py        â rename/replace with your Python Node Implementation
âââ python/impl/extension.py       â calls acquire_example_ipc_interface() on startup to load the plugin
âââ python/tests/                  â test modules go here
...
* The extension is enabled (green toggle) in **Window â Extensions**. If it was enabled before the build ran, disable and re-enable it.
...
compute(db)
     â
     âââº uri (or relevant inputs) changed? ââyesâââº teardown transport
     â                    C++: state.reset()    Python: custom_reset()
     â¼
try open: connect or listen / accept
     â         (retry next eval if not ready)
     â¼
transport ready? âânoâââº return false
     â    (recv: often "no full message yet")
     yes
     â¼
...
4. Wire the graph.
   ...
   * On Playback Tick `execOut` â Receive External Step `execIn`
   * Receive External Step `execOut` â Send Simulation Clock `execIn`

   Data:

   * Isaac Read Simulation Time `simulationTime` â Send Simulation Clock `simulationTime`
